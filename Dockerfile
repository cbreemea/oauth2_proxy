...
COPY --from=builder /go/src/github.com/pusher/oauth2_proxy/cbre/landx/templates /

USER 2000:2000

ENTRYPOINT ["/bin/oauth2_proxy",  "-config", "/test.cfg", "-custom-templates-dir", "/templates"]
